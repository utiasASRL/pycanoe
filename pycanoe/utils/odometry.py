...
def plot_loc_stats(
    ...
    # Note that the axes have y forward (longitudinal), x right (lateral), z up (vertical)
    axs[0, 0].hist(e[:, 1], bins=20)
    axs[0, 0].set_title("Longitudinal Error (m)")
    axs[0, 0].legend([f"median: {np.median(e[:, 1]):.2f} m"], loc="upper right")
    axs[0, 1].hist(e[:, 0], bins=20)
    axs[0, 1].set_title("Lateral Error (m)")
    axs[0, 1].legend([f"median: {np.median(e[:, 0]):.2f} m"], loc="upper right")
    axs[0, 2].hist(e[:, 2], bins=20)
    axs[0, 2].set_title("Vertical Error (m)")
    axs[0, 2].legend([f"median: {np.median(e[:, 2]):.2f} m"], loc="upper right")
    axs[1, 0].hist(e[:, 4], bins=20)
    axs[1, 0].set_title("Roll Error (deg)")
    axs[1, 0].legend([f"median: {np.median(e[:, 4]):.2f} deg"], loc="upper right")
    axs[1, 1].hist(e[:, 3], bins=20)
    axs[1, 1].set_title("Pitch Error (deg)")
    axs[1, 1].legend([f"median: {np.median(e[:, 3]):.2f} deg"], loc="upper right")
